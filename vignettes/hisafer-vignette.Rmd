...
There is only one build function, and it only takes a single arguement: `build_hisafe()`. This function takes a `hip` object as an input and builds the actual folders/files on your computer that Hi-sAFe will read to run. These directories will be created in the location specififed in the `path` portion of the `hip` object. 

```{r, eval = FALSE}
build_hisafe(hip = hip_exp)
```

In addition to building the folder/file structure, `build_hisfe()` also creates a map of each scene within the respective simulation folders. It is highly recommended to check these maps prior to running your simulations to ensure that the scene was built as intended.
...
`run_hisafe()`can also be used without a `hip` object to run Hi-sAFe simulations that were not created using hisafer. To do this, you must supply the `path` to the directory where the simulation folders are located. Just make sure the simulation folder/file structures are correct!
...
### 5. Data DIAGNOSTICS
Prior to performing any analyses using your `hop` data, it is highly recommended to check some basic diagnostics on your simluations to ensure that the simulations ran as expected. To do so, hisafer provides some basic diagnostic functions that allow you to compare your various simulations side by side:
...
You can also compare the major Hi-sAFe "cycles" among your simulations by using `plot_hisafe_cycle()` and `plot_hisafe_use()`. Supported "cycles" include carbon, water, nitrogen, and light. 
```{r, eval = TRUE}
plot_hisafe_cycle(hop = hop_exp, cycle = "carbon")
```
```{r, eval = FALSE}
plot_hisafe_cycle(hop = hop_exp, cycle = "nitrogen")
```
```{r, eval = FALSE}
plot_hisafe_cycle(hop = hop_exp, cycle = "water")
```
```{r, eval = TRUE}
plot_hisafe_cycle(hop = hop_exp, cycle = "light")
```
```{r, eval = FALSE}
plot_hisafe_use(hop = hop_exp, cycle = "carbon", years = 2014)
```
```{r, eval = FALSE}
plot_hisafe_use(hop = hop_exp, cycle = "nitrogen", years = 2014)
```
```{r, eval = FALSE}
plot_hisafe_use(hop = hop_exp, cycle = "water", years = 2014)
```
```{r, eval = TRUE}
plot_hisafe_use(hop = hop_exp, cycle = "light", years = 2014)
```
...
Sometimes, during the Analysis phase, you will realize that the values of **SimulationName** that you provided during the define phase are not quite what you wanted. To rename each **SimulationName** in your `hop` object, you can use `hop_rename()`.
...
Furthermore, just as for normal `hop` objects, you can also use `plot_hisafe_cycle()` and `plot_hisafe_use()` to compare the simulations in a `face` object. 
```{r, eval = TRUE}
plot_hisafe_cycle(hop = myface, cycle = "carbon")
```
```{r, eval = TRUE}
plot_hisafe_cycle(hop = myface, cycle = "light")
```
```{r, eval = TRUE}
plot_hisafe_use(hop = myface, cycle = "carbon", years = "all", simu.names = "Restinclieres-A2")
```
```{r, eval = TRUE}
plot_hisafe_use(hop = myface, cycle = "light", years = "all", simu.names = "Restinclieres-A2")
```

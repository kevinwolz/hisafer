...
So far we have only tried to edit Hi-sAFe parameters in the .SIM and .PLD files. `define_hisafe()` can also be used to edit parameters in the .TREE file. You must be careful, though, if your simluations have more than one tree, as any .TREE parameters passed to `define_hisafe()` will be applied to **all** .TREE files. Let's define a simple simulation where we edit `lueMax` in the .TREE file:
```{r}
hip <- define_hisafe(path      = "./simulations",
...
- `diag_hisafe_ts()` - plots a timeseries plot for each variable in the "annualtree", "annualplot", "trees", "plot", and  "climate" profiles of a `hop` object.
...
- `diag_hisafe_annualplot()` - plots a full range of tile plots for each variable in the "annualplot" profile of a `hop` object.

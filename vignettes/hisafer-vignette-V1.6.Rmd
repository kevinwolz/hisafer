---
title: "Introduction to hisafer 1.6.1"
...
remotes::install_github("kevinwolz/hisafer", ref = "updates2025")
```

Alternatively, if the vignette is desired to be included and installed within R:
```{r eval=FALSE}
remotes::install_github("kevinwolz/hisafer", ref = "updates2025" , build_vignettes = TRUE, dependencies = TRUE)
...
The template directory structure for the latest version of Hi-sAFe (v4.4) has all of the following contained within a single folder:
...
- `treeInterventions/` - a folder containing all of the individual .TTEC files that will be available for use
...
The main feature of a `hip` object is the `exp.plan`, but we'll wait to look at that until we have a more interesting simulation. The other components of the `hip` object contain exactly the three core arguments that we just passed to `define_hisafe()`: the template name/directory, the list of export profiles, and the path where the simulation is to be built. 
...
```{r} 
hip_params("simulationDateStart")
```

It looks like the agroforestry template defaults to running for 2 years. Let's start with a simple modification by changing that to 10 years and then take a look at the resulting `exp.plan`:

```{r}
hip <- define_hisafe(path     = "D:/SimulHisafeR/simulation/agroforestry",
                     template = "agroforestry",
                     simulationDateStart = "1995-01-01",
                     simulationDateEnd = "2004-12-31")
hip$exp.plan
```

Each row of the `exp.plan` table describes a single Hi-sAFe simulation, and each column provides the value of a Hi-sAFe input parameter. So, we can see here that we have a single simulation defined, with **simulationDateStart** and **simulationDateEnd** as the only modified parameters. All other Hi-sAFe parameters will remain unchagned from the template directory. If not provided manually, a default **SimulationName** is provided for each simulation within the `hip` object. This will become very important during analysis so we can specify which simulations we want to analyze.
...
```{r}
hip_sim <- define_hisafe(path     = "D:/SimulHisafeR/simulation/agroforestry",
                         template = "agroforestry",
                         latitude         = 60, 
                         northOrientation = 90)
hip_sim$exp.plan
...
```{r, eval = FALSE}
run_hisafe(path        = "D:/SimulHisafeR/simulations/lue"
...
mc.hip <- define_hisafe(path = example.path, template = "monocrop"    , SimulationName = "monocrop",sticsReport = 1)
af.hip <- define_hisafe(path = example.path, template = "agroforestry", SimulationName = "agroforestry", lueMax = 1.0)
pf.hip <- define_hisafe(path = example.path, template = "forestry",     SimulationName = "forestry",  lueMax = 1.0)
...
```
```{r, eval = TRUE, dpi = 250, fig.width = 10}
plot_hisafe_cycle_ts(hop = hop, cycle = "carbon", years = 2002)
```
```{r, eval = TRUE, dpi = 250, fig.width = 10}
plot_hisafe_cycle_ts(hop = hop, cycle = "nitrogen", years = 2002)
```
```{r, eval = TRUE, dpi = 250, fig.width = 10}
plot_hisafe_cycle_ts(hop = hop, cycle = "water", years = 2002)
```
```{r, eval = TRUE, dpi = 250, fig.width = 10}
plot_hisafe_cycle_ts(hop = hop, cycle = "light", years = 2002)
```
```{r, eval = TRUE, dpi = 250, fig.width = 10}
plot_hisafe_cycle_ts(hop = hop, cycle = "carbon-increment", years = 2002)
```
```{r, eval = TRUE, dpi = 250, fig.width = 10}
...
```{r, eval = TRUE, dpi = 250, fig.width = 10}
plot_hisafe_cycle_bar(hop = hop, cycle = "water", freq = "month", years = 2002)
```

```{r, eval = TRUE, dpi = 250, fig.width = 10}
plot_hisafe_cycle_bar(hop = hop, cycle = "water", freq = "day", years = 2002, months = 4:5)
```
